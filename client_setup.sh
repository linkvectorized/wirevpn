...
# ── 3b. Install wirevpn CLI ───────────────────────────────────────────────────
echo ""
echo "==> Installing wirevpn command..."
sudo mkdir -p /usr/local/bin
...
EOFWIREVPN
sudo chmod 755 /usr/local/bin/wirevpn
printf "   $PASS wirevpn command installed — use: sudo wirevpn [up|down|status]\n"
...
printf "    Connect:     ${CYAN}sudo wirevpn up${NC}\n"
printf "    Disconnect:  ${CYAN}sudo wirevpn down${NC}\n"
printf "    Status:      ${CYAN}sudo wirevpn status${NC}\n"
if [ "$PLATFORM" = "macos" ]; then
